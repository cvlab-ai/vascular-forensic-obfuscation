...
from angio_gen.utils.grad_cam import HospitalClassifierGradCAM


def save_gradcam_results(model, datamodule, gradcam, output_root):
    ...
    for images, labels in tqdm(test_loader):
        ...
        heatmaps = gradcam.generate_heatmap(images, labels)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--layer_id", type=int, default=11, help="Layer ID")
    ...
    gradcam = HospitalClassifierGradCAM(model, layer_id=args.layer_id)
    
    save_gradcam_results(model, datamodule, gradcam, f"hospital_clf_gradcam_layer_{args.layer_id}")
    save_gradcam_results(model, datamodule_anon, gradcam, f"hospital_clf_anon_gradcam_layer_{args.layer_id}")
    print("Done.")